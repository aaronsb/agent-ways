...
- **Parent-boost**: once an ancestor way has fired in the session, an in-domain child's semantic bar is lowered from `τ_s` to `(τ_s × parent_threshold_multiplier).max(parent_boost_floor)` — by default `max(0.5 × 0.8, 0.30) = 0.40`. The multiplier (0.8) is the boost; the floor (0.30) caps the cascade; both in probability space. This is how progressive disclosure amplifies in-domain children; see [hooks-and-ways/matching.md](../../../../docs/hooks-and-ways/matching.md).
