...
- **Concurrency:** atomic-write plus compare-and-swap. Each writer writes to `<file>.tmp`, renames into place, and retries on conflict. Two simultaneous fresh sessions racing for `alpha`: first writer wins; the loser reads-after-conflict and takes `beta`.
