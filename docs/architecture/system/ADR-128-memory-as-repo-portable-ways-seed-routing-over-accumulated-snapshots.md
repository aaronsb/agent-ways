...
Three failure modes follow from accepting that default.
...
- **Idempotent, versioned seeding** — the hook is hash-verified and version-aware. Template improvements ship as `seed-version` bumps; prior-version seeds auto-migrate with diff preservation. Nothing is silently lost.
...
- **Compaction becomes a re-routing trigger** — the periodic memory-compaction cycle rewrites `MEMORY.md` between sessions; our hash check detects it, the hook re-seeds + writes a diff, and the drift-review way fires at next session with an actionable triage prompt. Compaction output gets *converted* into repo artifacts (ways, ADRs, issues) or explicitly discarded — not silently re-consolidated back into memory. The harness's memory-tidying work becomes an input to our discipline, not a competitor.
...
- **Review cost per compaction cycle** — when compaction runs, Claude is prompted at next session-start to triage the diff. If the user just wants a quick task, this pulls attention into memory review first. Mitigation: the drift-review way can be dismissed per-session (mark diff `.reviewed` without actually reviewing), and users who want no compaction at all can toggle it off via `/memory`.
