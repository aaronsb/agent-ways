...
- **Idempotent, versioned seeding** — the hook is integrity-verified (byte-equality against the binary's embedded canonical) and version-aware. Template improvements ship as `seed-version` bumps; prior-version seeds auto-migrate with diff preservation. Nothing is silently lost.
