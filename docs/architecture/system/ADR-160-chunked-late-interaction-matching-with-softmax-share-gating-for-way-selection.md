...
- **Resident model daemon.** Deferred. Unnecessary for the embedding tier — batched embedding suffices for the load. Relevant only if a larger reasoning model is later introduced as a reranker. ADR-189 (Proposed) introduces a cross-encoder reranker behind this matcher and takes up the daemon on that condition.
