...
5. **Transport and identity.** The server runs as `attend mcp` over stdio, launched by the client once per session. Identity resolves per connection through the ADR-171 derivation (pid ancestry to the session record, the record's `cwd` as origin path, the target directories ADR-184 item 8 names). The server memoizes the tuple for the connection and never for the process, so a future shared transport inherits the rule. `status` reports the tuple and its `resolved` flag. `send` and `reply` refuse under `resolved: false` and return the cause, matching the drain's refusal in ADR-172 item 4, so a fallback persona is never written to the bus.
