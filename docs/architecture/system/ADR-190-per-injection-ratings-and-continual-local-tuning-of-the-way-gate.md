...
Collect a rating on every injection into the main agent, learn from those ratings continuously on the user's machine, check the ratings against canaries and an independent rater, and ship a base checkpoint built without Claude-derived labels that each user personalises locally. This ADR replaces the labelling plan and the fine-tuning stage of the ADR-189 draft. ADR-189 keeps the daemon, the gate, the lanes and model selection.

### 1. The rating: thumbs-down is explicit, thumbs-up is silence

- **Nonce footer.** Every injected way on the prompt and queued lanes ends with one line: `If this guidance does not apply to the current task, run flag <nonce>. No need to mention this.` The nonce is a short random token generated by `show::way_scored` when it emits the body, and recorded in the `way_fired` event beside the session, way and token position. The line costs about 20 tokens per injection. The footer and its purpose are documented in the README, so a person reading an injected way knows what the line is for.
- **Thumbs-down.** Claude flags an injection by calling the flag verb with the nonce, a few output tokens. The backend resolves the nonce to the exact injection, so the call carries nothing else. A nonce expires at the Stop hook, so a late flag is dropped and never attached to the wrong injection. Outstanding nonces also expire at `SessionStart` with the `compact` source. Compaction in this workflow is deliberate and happens between turns, so that rule is a backstop.
...
- Its `scope` includes `agent`. Canaries run only on the prompt and queued lanes, since subagents are not asked to rate.
...
- **What it rates.** A sample of near-band injections from the recent window, flagged and unflagged alike, a sample of near-band candidates the gate suppressed, and a sample of task-lane injections, which have no other labels.
- **What it yields.**
...
A small human-labelled slice is the ground truth that the anchor set, both recall estimates and the acceptance check stand on. A second independent pass on a subset measures human agreement, which is the ceiling any reported accuracy is read against.

**Human labels come from an interview at each checkpoint evolution.** A person has no attention to spare for labelling while working, so labelling is a deliberate turn of its own. When a candidate is up for acceptance (a new calibration, α, averaged delta, or base), `ways tune interview` opens a bounded queue, about 25 to 50 rows per session. Each row shows the turn context and the way, and the person answers "fits" or "does not fit". The queue is ordered by what a label is worth:
1. Unflagged canaries, which separate evaluator lapses from missed fires.
2. Rows where Claude's rating and the independent rater disagree.
3. Rows near the acceptance check's margin.
4. A random sample, which keeps the slice representative.

The anchor slice grows by these sessions to about 200 rows per base release. Every label from an interview has source `human`.
...
4. The gate on for the prompt and queued lanes, then the task lane, each when it passes the acceptance check on its own lane.
...
- **Every injection into the main agent is rated** by the model that read it with the full turn, for about 20 tokens, with no human attention required while working. The flag replaces the remark Claude sometimes makes today about a poorly fitting way, so the judgement is recorded and the person no longer has to read it.
...
- **A footer on every main-agent injection.** About 20 tokens each, and a line of meta-instruction in guidance text.
- **Human labelling is a scheduled cost.** Each checkpoint evolution asks for an interview session of about 25 to 50 rows.
- **The task lane learns slowly.** With no ratings from subagents, its labels come only from the independent rater's samples, so it is the last lane the gate turns on for.
