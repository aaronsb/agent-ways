...
- **recommendation:** Not a bug to fix in the way file itself (the attend trigger is correctly shaped per ADR-114's schema), but flag for tracking: this way is currently unreachable via any affordance since the motivation/time-based sensor that would emit the 'reflection-overdue' signal (per ADR-119/123) hasn't been built. Either build that sensor, or fold this way's distinct guidance into context-pressure.md until it does, so the repo doesn't carry a dormant handler indefinitely.
