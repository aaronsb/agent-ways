...
`from` is `claude:<session-id>` or `external:<user>@<terminal>`. The prefix names the implementation that wrote the file. The `re:` field is a tagged optional field: the parser in `tools/attend/src/cmd/inbox.rs` (`parse_signal`) consumes it only when the value matches the signal-id character class, so prose that begins with `re:` falls through as message text. Three parsers exist for this line (`attend/inbox.rs`, `sensor-peers/lib.rs`, `attend-chat/signal.rs`) and two writers; they are kept in lockstep by comments (ADR-136, Neutral consequences).
...
`from_id` and `to` share one id space: session ids for agents, sanitized usernames for humans. Display names and Greek ordinals never appear in these fields (ADR-171: ordinals are presentation, never keys). Every value is confined to `[A-Za-z0-9_@.,:/#*-]+`, which excludes `|`, whitespace, and control characters, so the tagged-field parse cannot be broken by a value.
...
- `session_resolved == false`: no Claude session owns the process. `from_kind = human`, `from_id = sanitize_id_component($USER)`, the same member id ADR-170 uses for channel membership. attend-chat always takes this branch.
...
Multi-recipient fan-out (ADR-136 Decision 4) writes one file per destination; each file carries the full addressee set, as RFC 5322 `To` does, so any recipient can see who else received it.
...
A session S is party to a message M when any of the following holds: S's session id is in `M.to`; `M.from_id` is S's session id (own messages already mark without delivering); M carries `re` and the message it replies to is one S is party to, followed through the `re` chain to the root. The `re` chain is resolvable because the referenced signals sit in the same scan directories and are never reaped by age. A channel message (`to = #<name>`) reaches S only if S joined the channel, so the drain treats channel traffic as party by membership. Broadcast traffic (`to = *`) is party only through the sender or the reply chain, and that was the case the 2026-09-19 session hit: ten broadcast messages between two other agents.
...
1. Shared parse and format for the line (or lockstep edits at `attend/src/cmd/inbox.rs`, `sensor-peers/src/lib.rs`, `attend-chat/src/signal.rs`), with round-trip tests for legacy and tagged records, the prose fence, and the legacy derivation.
2. `cmd_send`: resolve `from_kind` and `from_id` from `attend_session::identity()`, build `to` from the routing branch, accept `--on-behalf-of <id>` on `send` and `reply` in `cli.rs`. `attend-chat` `write_signal`: `from_kind = human`, `from_id = human_member_id()`, `to` from the channel.
3. `cmd_reply`: look up the parent by id to set `to`.
4. Readers: `Drained` and `render_drain_reason`, the sensor's Monitor line and digest, `cmd_inbox` and `cmd_inbox_read`.
5. The three synchronized messaging docs (`skills/attend/SKILL.md`, `tools/sensor-disclosure/src/disclosures/messaging.md`, `hooks/ways/softwaredev/environment/attend/attend.md`) gain one sentence each on `--on-behalf-of` and on what the header line means, per the ADR-136 lockstep rule.
